export module fab.ImageDrawable;
...
namespace fab {
	...
	public:
		...
		inline virtual void draw(sdl::SDLBatchRenderPass& rp, float x, float y, float w, float h, float winW, float winH, float scX = 1, float scY = 1, float rotZ = 0, const sdl::Color* tint = &sdl::COLOR_STANDARD, sdl::RenderMode pipeline = sdl::RenderMode::NORMAL) override {
			drawImpl(rp, (x + (0.5f * w)) / winW, (y + (0.5f * h)) / winH, scX * w / winW, scY * h / winH, rotZ, tint, pipeline);
		}
		inline virtual void drawCentered(sdl::SDLBatchRenderPass& rp, float x, float y, float w, float h, float winW, float winH, float scX = 1, float scY = 1, float rotZ = 0, const sdl::Color* tint = &sdl::COLOR_STANDARD, sdl::RenderMode pipeline = sdl::RenderMode::NORMAL) {
			drawImpl(rp, x / winW, y / winH, scX * w / winW, scY * h / winH, rotZ, tint, pipeline);
		}
		inline void drawCenteredFull(sdl::SDLBatchRenderPass& rp, float x, float y, float winW, float winH, float scX = 1, float scY = 1, float rotZ = 0, const sdl::Color* tint = &sdl::COLOR_STANDARD, sdl::RenderMode pipeline = sdl::RenderMode::NORMAL) {
			drawCentered(rp, x, y, texW, texH, winW, winH, scX, scY, rotZ, tint, pipeline);
		}
