...
namespace fbc {
	...
	public:
		SettingsDialog(FWindow& win): UIDialog(win, new ScreenSizeHitbox(win, 0.25, 0.25, 0.5, 0.5), cct.images.uiDarkPanelRound) {
		...
		UINavigation<SettingsDialogPage>& navigation = this->add(std::make_unique<UINavigation<SettingsDialogPage>>(win, new RelativeHitbox(*hb, -330, 0, 300, 100)));
		...
		inline static void openNew(FWindow& win) { win.openOverlay(std::make_unique<SettingsDialog>(win)); } // Displays a new Settings Dialog

		static strv windowScreenName(WindowMode mode);
	protected:
		inline SettingsDialogPage page(strv name) { return SettingsDialogPage(win, new RelativeHitbox(*hb, hb->w * 0.1f, hb->h * 0.1f, hb->w * 0.8f, hb->h * 0.7f), name); }
